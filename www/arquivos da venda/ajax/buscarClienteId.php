<?
		...
		if($idcliente && $acao == 'busca_cliente') {
			...
			$sql = "SELECT IdCliente, Nome, IdTipoBloqueio, CPF, Naturalidade, DataNascimento, RG, Telefone1, Pai, Mae, Endereco, TipoCliente, IdCidade, IdEstado FROM cliente";
			...
			if($rs->next()) {
				...
				$idcidade = $rs->get("IdCidade");
				$idestado = $rs->get("IdEstado");
				...
				$saldoconta = "";
				$sql = "SELECT SaldoAtual, Sigla FROM ctacorrente_cliente, moeda WHERE moeda.IdMoeda = ctacorrente_cliente.IdMoeda AND IdCliente = '$idcliente' AND SaldoAtual != 0.00";
				//echo $sql;
				$rs1 = $con->executeQuery ($sql);
				while($rs1->next()){
					$saldoconta .= $rs1->get('SaldoAtual').' '.$rs1->get('Sigla').'      ';
				}
				$rs1->close();
			}else{
				unset($idcliente);
			...
			$rs->close();	
				...
				$sql = "SELECT Fator FROM fator_pedra_cliente WHERE IdCliente = '$idcliente'";
				//echo $sql;
				$rs1 = $con->executeQuery ($sql);
				if($rs1->next())
					$fatorpedra = $rs1->get(0);
				$rs1->close();
			...
			echo $idcliente.'|'.
				...
				$isBloqueio.'|'.
				$fatorpedra.'|'.
				'<font color="red">'.$saldoconta.'</font>|'.
				$idcidade."|".
				$idestado;
		}
